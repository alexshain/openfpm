...
inline bool weighted_quadratic_fit(const std::vector<Point<2,double>>& pts,
    ...
    auto Wab = [](double r) -> double {
        double q = r / H;
        if (q <= 1.0) return (1.0 - 1.5*q*q + 0.75*q*q*q) * (10.0/(7.0*M_PI*H*H));
        else if (q <= 2.0) return (0.25*(2.0 - q)*(2.0 - q)*(2.0 - q)) * (10.0/(7.0*M_PI*H*H));
        else return 0.0;
    };
...
inline double get_alpha(auto a, const Point<2,double> &new_origin, particles & vd) {
    ...
    // Угол от центра к частице
    double theta = atan2(ry, rx);
    
    // Поворачиваем на 90 градусов чтобы ось x была касательной
...
inline double compute_first_deriv_of_lagrange(const std::vector<Point<2,double>>& points, double x_eval) {
    ...
    double first_deriv = (f_plus - f_minus) / (2 * h);
