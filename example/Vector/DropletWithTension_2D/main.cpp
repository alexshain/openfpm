...
const double MassFluid = rho_zero * dp * dp;

// Mass of the boundary particles
const double MassBound = rho_zero * dp * dp;
...
const double c1 = -3.0/M_PI/H/H/H;
const double d1 = 9.0/4.0/M_PI/H/H/H;
const double c2 = -3.0/4.0/M_PI/H/H/H;
...
inline double Pi(const Point<2,double> & dr, double rr2, Point<2,double> & dv, double rhoa, double rhob, double massb, double & visc)
{
	const double dot = dr.get(0)*dv.get(0) + dr.get(1)*dv.get(1);
	...
	while (part.isNext())
	{
		...
		if (vd.getProp<type>(a) != FLUID)
		{
			...
			while (Np.isNext() == true)
			{
				...
				Point<2,double> vb = vd.getProp<velocity>(b);
				...
				if (r2 < 4.0*H*H)
				{
					...
					vd.getProp<drho>(a) += massb*(dv.get(0)*DW.get(0)+dv.get(1)*DW.get(1));
				}
		...
		else
		{
			...
			while (Np.isNext() == true)
			{
				...
				if (r2 < 4.0*H*H)
				{
					...
					vd.getProp<drho>(a) += massb*(v_rel.get(0)*DW.get(0)+v_rel.get(1)*DW.get(1));
				}
...
void max_acceleration_and_velocity(particles & vd, double & max_acc, double & max_vel)
{
	...
	while (part.isNext())
	{
		...
		Point<2,double> acc(vd.getProp<force>(a));
		double acc2 = norm2(acc);

		Point<2,double> vel(vd.getProp<velocity>(a));
...
void verlet_int(particles & vd, double dt)
{
	...
	while (part.isNext())
	{
	    ...
	    if (vd.getPos(a)[0] < 0.0 || vd.getPos(a)[1] < 0.0 ||
		    vd.getPos(a)[0] > 0.02 || vd.getPos(a)[1] > 0.02 ||
...
inline void sensor_pressure(Vector & vd,
                            ...
                            openfpm::vector<Point<2,double>> & probes)
{
    Vcluster<> & v_cl = create_vcluster();
    ...
    for (size_t i = 0 ; i < probes.size() ; i++)
    {
		...
		if (vd.getDecomposition().isLocal(probes.get(i)) == true)
		{
			...
			Point<2,double> xp = probes.get(i);
...
int main(int argc, char* argv[])
{
	...
	probes.add({0.01, 0.01});  // центр домена, высота 5 мм
	probes.add({0.005, 0.005});

	Box<2,double> domain({0.0,0.0},{0.02,0.02});
	size_t sz[2] = {static_cast<size_t>((domain.getHigh(0) - domain.getLow(0)) / dp) + 1,
    				static_cast<size_t>((domain.getHigh(1) - domain.getLow(1)) / dp) + 1};
	...
	/*Box<3,double> fluid_box({0.1+dp/2.0,0.1+dp/2.0,0.1+dp/2.0},{0.3+dp/2.0,0.47-dp/2.0,0.2+dp/2.0});
	...
	while (fluid_it.isNext())
	{
	    ...
	    //if (dist <= sphere_radius) {
	        ...
	        vd.getLastPos()[0] = pos[0];
	        vd.getLastPos()[1] = pos[1];
	...
	openfpm::vector<Box<2,double>> holes;
	...
	while (t <= t_end)
	{
		...
		double write_interval = 0.001; // Интервал записи в секундах

		if (t >= write * write_interval)
		{
